...
$(function () {
  var effect = 'blind', itemText,
    editView = $('#editView'),
    printView = $('#printView').hide(),
    switchViewButton = $('#switchViewButton').button(),
    span = switchViewButton.children('span');

  switchViewButton.click(function () {
    var wantEdit = span.text() === 'Edit View';
    if (wantEdit) {
      span.text('Print View');
  ...
  //var items = ['banana', 'burritos', 'cereal', 'gatorade', 'lemonade',
...
mp.addTabs = function (containerId, tabIds, tabLabels) {
  ...
  container.tabs({
    select: function (event, ui) {
      ...
      $('#' + mp.day).accordion(
        'option', 'active', mp.meals.indexOf(mp.meal));
    }
  });
...
mp.createItem = function (text) {
  var itemDiv = $('<div>').addClass('item').text(text);
...
mp.dropItem = function (itemDiv, dayOrMeal) {
  ...
  $('#tabs').tabs('option', 'active', mp.days.indexOf(mp.day));

  // Open the accordion panel for the meal on which it was dropped.
  $('#' + mp.day).accordion('option', 'active', mp.meals.indexOf(mp.meal));
